#include "lined_charbuf.h"

#include <cstddef> 
#include <string_view>
#include <string>
#include <ranges> 
#include <iterator>
#include <compare> 
#include <deque> 

template<typename char_t> 
void inc_helper(size_t& line, size_t& col, LinedCharbuf<char_t>* theCharbuf){
  ++col;
  if (col >= theCharbuf->lines[line].size()){
    ++line; col=0;
  }
}

template<typename char_t> 
void dec_helper(size_t& line, size_t& col, LinedCharbuf<char_t>* theCharbuf){
  if (col == 0){
    --line; col = theCharbuf->lines[line].size();
  }
  --col; 
}

template<typename char_t> 
typename LinedCharbuf<char_t>::iterator& LinedCharbuf<char_t>::iterator::operator++(){
  inc_helper(line, col, this->theCharbuf);
  return *this;
}

template<typename char_t> 
typename LinedCharbuf<char_t>::iterator& LinedCharbuf<char_t>::iterator::operator--(){
  dec_helper(line, col, this->theCharbuf);
  return *this;
}

template<typename char_t> 
typename LinedCharbuf<char_t>::const_iterator& LinedCharbuf<char_t>::const_iterator::operator++(){
  inc_helper(line, col, this->theCharbuf);
  return *this;
}

template<typename char_t> 
typename LinedCharbuf<char_t>::const_iterator& LinedCharbuf<char_t>::const_iterator::operator--(){
  dec_helper(line, col, this->theCharbuf);
  return *this;
}

template<typename char_t> 
// erase `num` characters from line `line` starting at the `start` index (0 indexed)
// - default start is 0, default num is npos (i.e. +infty)
void LinedCharbuf<char_t>::erase(size_t line, size_t start, size_t num){
  const auto& curline = lines[line];
  if (num == std::string::npos){
    num = curline.size()-start-1;
  }
  curline = curline.substr(0,start) + curline.substr(start+num);
}
// insert all characters from `chars` into line `line` starting at index `start` (0 indexed) 
template<typename char_t>
void LinedCharbuf<char_t>::insert(size_t line, size_t start, std::basic_string<char_t> chars){
  const auto& curline = lines[line];
  curline = curline.substr(0, start) + chars + curline.substr(start);
}
// append all characters from `chars` into end of line `line` (0 indexed) 
template<typename char_t>
void LinedCharbuf<char_t>::append(size_t line, std::basic_string<char_t> chars){
  const auto& curline = lines[line];
  curline.pop_back();
  curline += chars + "\n";
}
template<typename char_t>
void LinedCharbuf<char_t>::append(size_t line, char_t ch){
  curline.pop_back();
  curline.push_back(ch);
  curline.push_back('\n');
}
// insert `num` blank lines before line `line`
template<typename char_t>
void LinedCharbuf<char_t>::insert_lines(size_t line, size_t num){
  lines.insert(lines.begin()+line, num, "\n");
}
// erase `num` blank lines starting from `line`
template<typename char_t>
void LinedCharbuf<char_t>::erase_lines(size_t line, size_t num){
  lines.erase(lines.begin()+line, lines.begin()+line+num);
}
